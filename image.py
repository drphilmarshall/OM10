...
class Imager(object):
    ...
    def __init__(self,survey=None):
        ...
        self.qflux=numpy.array([self.magnif[0]*self.qmag,self.magnif[1]*self.qmag])
        self.flat=0.8
        self.pa=0.0     
        ...
        self.fluct = exp(-9.-numpy.log(10.)*numpy.random.normal(0,self.errdepth,(self.imsize,self.imsize)))-1.
...
#        logfile = os.path.expandvars("$OM10_DIR/data/qso_mock_log.dat") ...
    ...
    def make(self,filters=['i'],Nepochs=1):
        ...
        skyflux = exp(-0.4*numpy.log(10.)*self.meandepth)
        ...
        for k in range(Nepochs):
            ...
            for kq in range(quasims-1):
                ...
                for i1 in range(imin,imax):
                    for i2 in range(jmin,jmax):
                        self.image += ( self.psf[self.midpsf+i1-ipos-1,self.midpsf+i2-jpos-1]*(1.-abs(dx))*(1.-abs(dy))
            ...
            # Noise map:
            self.var = self.image*(exp(9.-0.4*numpy.log(10)*self.errdepth)-1.)
