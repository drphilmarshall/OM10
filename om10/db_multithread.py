        ...
        * The treatment of QSOs may be flawed: the offset calculation has not
        ...
        if synthetic==True:
            ...
            for lens in self.sample:
                ...
                RF_Gmag_app = Process(target=ABFilterMagnitude, args=(Gfilter,sed, redshift))
                RF_Gmag_app.start()
                RF_Gmag_app.join()
                RF_Imag_app = Process(target=ABFilterMagnitude, args=(Ifilter,sed, redshift))
                RF_Imag_app.start()
                RF_Imag_app.join()
                RF_Zmag_app = Process(target=ABFilterMagnitude, args=(Zfilter,sed, redshift))
                RF_Zmag_app.start()
                RF_Zmag_app.join()
                RF_Gmag_app = RF_Gmag_app + offset + d.distance_modulus(redshift)
                RF_Imag_app = RF_Imag_app + offset + d.distance_modulus(redshift)
                RF_Zmag_app = RF_Zmag_app + offset + d.distance_modulus(redshift)
